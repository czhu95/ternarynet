...
class Model(ModelDesc):
    ...
    def _build_graph(self, input_vars):
        ...
        def new_get_variable(name, shape=None, **kwargs):
            ...
            if name is not 'W' or 'conv0' in v.op.name or 'fct' in v.op.name:
                return v
            else:
#                 return v
                return p_ternarize(v, PROP)
...
def get_data(train_or_test):
    ...
    ds = BatchData(ds, BATCH_SIZE / NUM_DEVICES, remainder=not isTrain)
    if isTrain:
        ds = PrefetchData(ds, 3, 2)
    return ds

def get_config(nsize):
    mod = sys.modules['__main__']
    basename = os.path.basename(mod.__file__)
    logdir = os.path.join('train_log',
                         'ter-cifar10-resnet-{}'.format(6*nsize+2),
                         'p', 'p-{}'.format(PROP))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p', help="Percentage", type=float, default=0.5)
    ...
    NUM_DEVICES = len(args.gpu.split(','))
